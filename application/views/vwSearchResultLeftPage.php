        ...
        <ul class="filter-list">
            ...
            <?php foreach ($refine_skills as $refine_skill) { ?>
                <?php if ($refine_skill['count']->count_number != '' && $refine_skill['count']->count_number != '0') { ?>
                    <li><a href="<?php echo base_url('search/role_wise_search/') . base64_encode($refine_skill['id']); ?>" target="_blank"><?php echo $refine_skill['skill']; ?> <span>( <?php echo $refine_skill['count']->count_number; ?> )</span></a></li>
                        <?php } ?>
                    <?php } ?>				
